...
process rmse_process {
  ...
  script:
    if (params.test)
        ...
        ./${params.rmse.tests.testScript} | tee $output
        """
    else
        """
        # Some useful stuff
...
workflow rmse {
    ...
    result_ = rmse_process(id_input_output_function_cli_) \
...
workflow test {
   ...
   main:
   params.test = true
   params.rmse.output = "rmse.log"

   Channel.from(rootDir) \
        | filter { params.rmse.tests.isDefined } \
        | map{ p -> new Tuple3(
                    "tests",
                    params.rmse.tests.testResources.collect{ file( p + it ) },
                    params
                )} \
        | rmse
