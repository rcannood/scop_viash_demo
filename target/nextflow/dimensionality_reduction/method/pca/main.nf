...
workflow pca {
    ...
    result_ = pca_process(id_input_output_function_cli_) \
...
workflow test {
   ...
   main:
   params.test = true
   params.pca.output = "pca.log"

   Channel.from(rootDir) \
        | filter { params.pca.tests.isDefined } \
        | map{ p -> new Tuple3(
                    "tests",
                    params.pca.tests.testResources.collect{ file( p + it ) },
                    params
                )} \
        | pca
